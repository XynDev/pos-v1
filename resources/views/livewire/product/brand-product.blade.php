    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-3">
                    ...
                    <table class="table-auto w-full dark:text-gray-300">
                        ...
                        @forelse($brands as $brand)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/20">
                                <td class="p-2">
                                    <div class="font-medium text-gray-800 dark:text-gray-100">{{ $brand->name }}</div>
                                </td>
                                <td class="p-2">
                                    <div>{{ $brand->slug }}</div>
                                </td>
                                <td class="p-2">
                                    <div class="flex justify-center items-center space-x-2">
                                        <button wire:click="edit({{ $brand->id }})" class="btn-sm bg-yellow-500 hover:bg-yellow-600 text-white">Edit</button>
                                        <button wire:click="delete({{ $brand->id }})" class="btn-sm bg-red-500 hover:bg-red-600 text-white">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                ...
                <div class="mt-4 px-3">
                    {{ $brands->links('components.pagination-numeric') }}
                </div>
            </div>
        </div>
    </div>
