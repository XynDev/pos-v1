    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <x-page-header>
            ...
            <x-slot name="actions">
                ...
                <button wire:click="create()" class="btn bg-indigo-600 hover:bg-indigo-700 text-white whitespace-nowrap">
                    ...
                    <span class="hidden xs:block ml-2">Tambah User</span>
                </button>
            </x-slot>
        </x-page-header>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-3">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/20">
                                ...
                                <td class="p-2">
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($user->getRoleNames() as $role)
                                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-indigo-100 dark:bg-indigo-800/30 text-indigo-800 dark:text-indigo-300">
                                                {{ $role }}
                                            </span>
                                        @endforeach
                                    </div>
                                ...
                                <td class="p-2">
                                    <div class="flex justify-center items-center space-x-2">
                                        <button wire:click="edit({{ $user->id }})" class="btn-sm bg-yellow-500 hover:bg-yellow-600 text-white">Edit</button>
                                        <button wire:click="delete({{ $user->id }})" class="btn-sm bg-red-500 hover:bg-red-600 text-white">Hapus</button>
                                    </div>
                                </td>
                            </tr>
                        @empty
                ...
                <div class="mt-4 px-3">
                    {{ $users->links('components.pagination-numeric') }}
                </div>
            </div>
        </div>
    </div>
